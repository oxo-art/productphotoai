      ...
      <Card className={`${getThemeStyle('card')} ${getThemeStyle('shadow')} transition-all duration-1000`}>
        <CardContent className="p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className={`text-xl font-semibold ${getThemeStyle('text').primary} ${getThemeStyle('typography')}`}>Upload Your Image</h3>
          </div>
          ...
          {!uploadedImage ? (
            ...
            >
              ...
              <h3 className={`text-2xl ${getThemeStyle('typography')} mb-4 ${getThemeStyle('text').primary}`}>Drop your image here</h3>
              <p className={`${getThemeStyle('text').secondary} mb-6 text-lg ${getThemeStyle('typography')}`}>
                Drag and drop your image, or click to browse
              </p>
              <Button 
                onClick={openFileDialog}
                className={`${getThemeStyle('buttonPrimary')} text-white px-8 py-3 rounded-lg ${getThemeStyle('typography')} transition-all duration-300 hover:scale-105`}
              >
                Choose File
              </Button>
              <input
            ...
            <div className="space-y-6">
              <div className="relative group overflow-hidden rounded-xl">
                <img
                  ...
                  alt="Uploaded image"
                  className={`w-full h-auto object-contain rounded-xl border-2 border-amber-500/40 ${getThemeStyle('shadow')}`}
                  style={{
              ...
              <Button
                variant="outline"
                onClick={openFileDialog}
                className={`w-full border-amber-500/40 ${getThemeStyle('text').primary} ${getThemeStyle('typography')} hover:bg-amber-500/10 hover:border-amber-500/60 transition-all duration-300`}
              >
      ...
      <Card className={`${getThemeStyle('card')} ${getThemeStyle('shadow')} transition-all duration-1000`}>
        <CardContent className="p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className={`text-xl ${getThemeStyle('typography')} ${getThemeStyle('text').primary}`}>Describe Your Vision</h3>
          </div>
          ...
          <div className="space-y-6">
            <Textarea
              ...
              className={`min-h-[120px] ${getThemeStyle('input')} ${getThemeStyle('text').primary} ${getThemeStyle('typography')} placeholder:text-amber-300/50 resize-none rounded-xl`}
            />
            
            {/* Prompt Suggestions */}
            <div className="space-y-3">
              <p className={`${getThemeStyle('text').secondary} text-sm ${getThemeStyle('typography')}`}>Quick suggestions:</p>
              <div className="flex flex-wrap gap-2">
                {promptSuggestions.map((suggestion, index) => (
                  <button
                    key={index}
                    onClick={() => addPromptSuggestion(suggestion)}
                    className={`px-4 py-2 ${getThemeStyle('button')} ${getThemeStyle('text').secondary} hover:${getThemeStyle('text').primary} ${getThemeStyle('typography')} text-sm transition-all duration-200 hover:scale-105 rounded-lg`}
                  >
                    {suggestion}
                  </button>
                ))}
            ...
            <Button 
              ...
              className={`w-full ${getThemeStyle('buttonPrimary')} text-white py-4 text-lg ${getThemeStyle('typography')} rounded-xl ${getThemeStyle('shadow')} transition-all duration-300 hover:scale-105`}
        ...
        <Card className={`${getThemeStyle('card')} ${getThemeStyle('shadow')} transition-all duration-1000`}>
          <CardContent className="p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className={`text-xl ${getThemeStyle('typography')} ${getThemeStyle('text').primary}`}>Your Transformed Images</h3>
            </div>
              ...
              {generatedImages.map((image, index) => (
                <div key={index} className="relative group overflow-hidden rounded-xl">
                  ...
                  <div className="absolute top-4 right-4 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <button
                      className={`h-12 w-12 rounded-xl ${getThemeStyle('buttonPrimary')} p-[2px] transition-all duration-200 ${getThemeStyle('shadow')} hover:shadow-xl hover:scale-110`}
...
export default ArtisticImageUpload;
